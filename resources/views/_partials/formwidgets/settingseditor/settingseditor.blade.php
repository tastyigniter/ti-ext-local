<div class="accordion" id="accordionSettings">
    @foreach($settings as $group => $setting)
        <div class="accordion-item border rounded-3 mb-2">
            ...
            @else
                <h2 class="accordion-header">
                    <a class="accordion-button border-0 rounded-3 collapsed"  href="{{ admin_url('locations/settings/' . $this->model->id . '?general=' . $setting->code) }}" aria-expanded="false" aria-controls="collapse{{ ucfirst($setting->code) }}">
                        @lang($setting->label)
                    </a>
                </h2>
            @endif
