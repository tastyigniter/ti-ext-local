...
it('creates a review for a completed sale', function() {
...
it('throws exception when creating review for incomplete sale', function() {
    $reviewable = Order::factory()->create();

    $data = ['quality' => 5, 'delivery' => 4, 'service' => 3, 'review_text' => 'Great service!'];

    expect(fn() => Review::leaveReview($reviewable, $data))->toThrow(ApplicationException::class);
});

it('throws exception when creating duplicate review', function() {
...
it('returns correct reviewable type options', function() {
...
it('finds reviewable by sale type and id', function() {
...
it('applies reviewable query scope correctly', function() {
    $order = Order::factory()->create();
...
it('returns review dates as an array', function() {
    $review = Review::factory()->create(['created_at' => now()]);

    $result = $review->getReviewDates();

    expect($result)->toBeArray()
        ->and($result)->toContain($review->created_at->format('F Y'));
});

it('returns null if location id is not provided for score calculation', function() {
    $result = Review::getScoreForLocation(null);

    expect($result)->toBeNull();
});

it('calculates correct score for location', function() {
...
it('configures review model correctly', function() {
    $review = new Review;
